...
ALL_COLS = [
    "user_id", "item_id", "rec_algo",
    "is_click", "is_cvr", "is_click_detail", "is_click_stock", "stay_time",
    "p_date", "fav_securities", "recent_stocks", "interest_keywords",
    "follow_authors", "is_new_user", "hold_stocks", "hist_hold_stocks",
    "historical_click_items", "asset_level", "last_login_date",
    "city", "investment_horizon", "invest_style", "theme_interest",
    "industry_interest", "fund_favorites", "item_type",
    "roleneeds_first_label", "roleneeds_second_label", "invest_label",
    "invest_label_second", "invest_label_third", "quality_score_label",
    "stock_list", "entity_words_label", "item_entities_v3",
    "author_id", "author", "source_name", "emb_id",
    "wordnum", "answerscore", "has_picture", "has_video"
]
...
def run_training(data_path: Path, feature_config: Path, model_config: Path, weights_path: Path) -> None:
...
def main() -> None:
    ...
    # 3. Rust predictions
    run_rust_inference(
        feature_cfg_path, model_cfg_path, weights_path, test_csv, rust_preds_csv
    )
