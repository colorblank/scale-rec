...
def _parse_task_config(raw):
    towers=[TowerConfig(t["name"],t.get("hidden_dims",[]),t.get("output_dim",1),Activation.from_str(t.get("activation","relu"))) for t in raw.get("towers",[])]
...
class ModelConfig:
    ...
    ctr_hidden_dims:list[int]=field(default_factory=list); cvr_hidden_dims:list[int]=field(default_factory=list)
    token_dim:int=64; num_tokens:int=8; num_blocks:int=2; block_size:int|None=None
    use_lite:bool=False; hidden_factor:float=1.0; num_basis:int=4; rank:int=16; use_siamese:bool=False
    ...
    def build(self, features: list[tuple[str,int,int]], tokenizer: FeatureTokenizer|None=None):
        ...
        if m=="lr": return LogisticRegression(features)
            ...
            return UniMixerModel(tokenizer=tokenizer,token_dim=self.token_dim,num_tokens=self.num_tokens,num_blocks=self.num_blocks,block_size_opt=self.block_size,use_lite=self.use_lite,hidden_factor=self.hidden_factor,num_basis=self.num_basis,rank=self.rank,task_config=self.task_config,use_siamese=self.use_siamese)
