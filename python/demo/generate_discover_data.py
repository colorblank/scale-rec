...
"""discover-main-sort 场景合成训练数据生成。

覆盖 Item 18 字段 + User 21 字段 + 4 标签字段 + const_dummy。
所有格式匹配真实场景: JSON 数组、结构化字符串(|/#)、RQ-VAE 编码等。
"""
...
# ═══════════════════════════════════════════
# 受控词汇表 (与 feature_config_discover.yaml 的 DictMapper mapping 一致)
# ═══════════════════════════════════════════
...
def _compute_labels(
    ...
    is_click_stock = 1 if ctr == 1 and stock_overlap == 1 and rng.random() < 0.5 else 0
    return ctr, cvr, is_click_detail, is_click_stock
...
def make_item(item_id: int, rng: random.Random) -> dict:
    ...
    kw = rng.choice(INTEREST_KEYWORDS)
    word = rng.choice(ENTITY_WORDS)
    stock_code = rng.choice(stock_codes)

    title = rng.choice(TITLE_TEMPLATES).format(stock=stock_code, kw=kw, word=word)
    content = f"${stock_code}$ {title}。近期{kw}板块表现活跃，{word}信号明显。建议投资者关注。"
    ...
    return {
        ...
        "title": title,
        "content": content,
        "insight": "" if rng.random() < 0.7 else f"{kw}核心观点：{word}趋势明确",
...
def make_user(user_id: int, rng: random.Random) -> dict:
    ...
    return {
        ...
        "scene": 0,
        "stay_time": int(rng.expovariate(1.0 / 600) + 30),
        "p_date": "20260331",
...
def main():
    ...
    all_cols = source_cols + label_cols
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        ...
        for uid in range(n_users):
            ...
            for _ in range(rows_per_user):
                ...
                ctr, cvr, is_click_detail, is_click_stock = _compute_labels(
                    rng, uid, it["quality_score_label"], stock_overlap, entity_overlap
                )
                ctr_sum += ctr
                cvr_sum += cvr
                ...
                for col in source_cols:
                    val = it.get(col, uf.get(col, ""))
                    row.append(val)
                row.extend([ctr, cvr, is_click_detail, is_click_stock])
                w.writerow(row)

    ctr_rate = ctr_sum / total_rows
    cvr_rate = cvr_sum / total_rows
    print(f"[Generate] {total_rows} rows, {len(all_cols)} cols")
    print(f"  CTR rate={ctr_rate:.3f}  CVR rate={cvr_rate:.3f}")
    print(f"  → {out_path}")
