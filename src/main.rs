...
fn main() -> Result<()> {
    ...
    let yaml = std::fs::read_to_string("examples/feature_config.yaml").expect("Failed to read config");
    ...
    let tokenizer_features: Vec<(String, usize, usize)> = embed_features.iter().map(|(name, emb)| {
        println!("  {} -> vocab={}, dim={}", name, emb.vocab_size, emb.embed_dim);
    ...
    raw_inputs.insert("user_tags".into(), Arc::new("sports#1|gaming#0.8".to_string()));
    raw_inputs.insert("item_price".into(), Arc::new(5999.0f32));
    let pre_result = dag.execute(&raw_inputs).map_err(|e| candle_core::Error::Msg(e))?;
    println!("  Sources: {} | Computed: {}", pre_result.source_names.len(), pre_result.computed_names.len());

    let batch_size = 1usize;
    let feature_tensors: HashMap<String, Tensor> = tokenizer_features.iter().map(|(name, _, _)| {
    ...
    let token_dim = 64;
    let num_tokens = 8;
    let tokenizer = FeatureTokenizer::new(vb.pp("tokenizer"), &tokenizer_features, token_dim, num_tokens)?;
    let task_config = MultiTaskConfig {
        towers: vec![
            TowerConfig { name: "ctr".into(), hidden_dims: vec![128], output_dim: 1, activation: Activation::Relu },
            TowerConfig { name: "cvr".into(), hidden_dims: vec![128], output_dim: 1, activation: Activation::Relu },
        ],
        relations: vec![],
    };
    let model = UniMixerModel::new(tokenizer, token_dim, num_tokens, 2, None, false, 1.0, 4, 16, &task_config, false, vb.pp("unimixer"))?;
    println!("\n[Model] UniMixer: embed_dim={}, block_size={}", model.embed_dim, model.block_size);
