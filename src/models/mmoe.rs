...
impl MMoE {
    ...
    pub fn new(vb: VarBuilder, features: &[(String, usize, usize)], shared_bottom_dims: &[usize], num_experts: usize, expert_hidden_dims: &[usize], expert_output_dim: usize, task_configs: &[(String, Vec<usize>)]) -> Result<Self> {
        ...
        let (shared_bottom, shared_output_dim) = if shared_bottom_dims.is_empty() {
            ...
            let mlp = Mlp::new(vb.pp("shared_bottom"), embeddings.total_dim, &shared_bottom_dims[..shared_bottom_dims.len() - 1], output_dim, Activation::Relu)?;
            (Some(mlp), output_dim)
        };
        let mut experts = Vec::with_capacity(num_experts);
        for e in 0..num_experts {
            experts.push(Mlp::new(vb.pp(format!("expert_{}", e)), shared_output_dim, expert_hidden_dims, expert_output_dim, Activation::Relu)?);
        }
        let num_tasks = task_configs.len();
        let mut gate_linears = Vec::with_capacity(num_tasks);
        for t in 0..num_tasks { gate_linears.push(linear(shared_output_dim, num_experts, vb.pp(format!("gate_{}", t)))?); }
        ...
        for (t, (name, tower_dims)) in task_configs.iter().enumerate() {
            ...
            task_towers.push(Mlp::new(vb.pp(format!("task_{}_tower", t)), expert_output_dim, tower_dims, 1, Activation::Relu)?);
        }
        Ok(Self { embeddings, shared_bottom, num_experts, experts, expert_output_dim, gate_linears, task_towers, task_names })
    }
}
...
impl Model for MMoE {
    fn forward(&self, x_inputs: &HashMap<String, Tensor>) -> Result<HashMap<String, Tensor>> {
        ...
        let shared_output = match &self.shared_bottom { Some(b) => b.forward(&concat)?, None => concat };
        let mut expert_outs = Vec::with_capacity(self.num_experts);
        for expert in &self.experts { expert_outs.push(expert.forward(&shared_output)?.unsqueeze(1)?); }
