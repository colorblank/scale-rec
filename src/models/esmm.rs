...
impl ESMM {
    pub fn new(vb: VarBuilder, features: &[(String, usize, usize)], shared_bottom_dims: &[usize], ctr_hidden_dims: &[usize], cvr_hidden_dims: &[usize]) -> Result<Self> {
        ...
        let ctr_tower = TaskTower::new(&TowerConfig { name: "ctr".into(), hidden_dims: ctr_hidden_dims.to_vec(), output_dim: 1, activation: Activation::Relu }, shared_output_dim, vb.pp("ctr_tower"))?;
        let cvr_tower = TaskTower::new(&TowerConfig { name: "cvr".into(), hidden_dims: cvr_hidden_dims.to_vec(), output_dim: 1, activation: Activation::Relu }, shared_output_dim, vb.pp("cvr_tower"))?;
        Ok(Self { embeddings, shared_bottom, ctr_tower, cvr_tower })
    }
}
