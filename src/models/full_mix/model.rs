//! Full-Mix model: FeatureTokenizer + FullMix blocks + mean pooling + task towers.

use super::block::FullMixBlock;
...
/// Dense Full-Mix model with parameterized token mixing and GLU P-FFNs.
pub struct FullMixModel {
    tokenizer: FeatureTokenizer,
    blocks: Vec<FullMixBlock>,
    task_towers: Option<MultiTaskTower>,
    output_head: Option<OutputHead>,
}

impl FullMixModel {
    /// Construct a Full-Mix model.
    pub fn new(
        tokenizer: FeatureTokenizer,
        token_dim: usize,
        num_tokens: usize,
    ...
    /// Construct a Full-Mix model with a native output contract.
