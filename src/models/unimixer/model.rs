...
impl UniMixerModel {
    ...
    pub fn new(
        ...
        for i in 0..num_blocks {
            let block = UniMixerBlock::new(embed_dim, block_size, token_dim, num_tokens, use_lite, hidden_factor, num_basis, rank, vb.pp(format!("blocks.{}", i)))?;
            blocks.push(block);
        }
        let task_towers = MultiTaskTower::new(task_config, embed_dim, vb.pp("task_towers"))?;
        let final_norm = if use_siamese { Some(SiameseNorm::new(embed_dim, 1e-5, vb.pp("final_norm"))?) } else { None };
        Ok(Self { embed_dim, block_size, use_siamese, temperature: 1.0, tokenizer, blocks, task_towers, final_norm })
    }

    pub fn set_temperature(&mut self, t: f64) { self.temperature = t; }

    pub fn forward_with_temperature(&self, x_inputs: &HashMap<String, Tensor>, temperature: f64) -> Result<HashMap<String, Tensor>> {
        ...
        let output = if self.use_siamese {
            ...
            for block in &self.blocks {
                ...
                if let BlockOutput::Siamese(_, new_x_bar, new_y_bar) = res { x_bar = new_x_bar; y_bar = new_y_bar; x = x_bar.clone(); }
                else { candle_core::bail!("Expected Siamese output"); }
            }
            let final_norm = self.final_norm.as_ref().unwrap();
            let fusion_res = final_norm.forward(&x_bar, &y_bar, None)?;
            if let SiameseNormOutput::Fused(fused_out) = fusion_res { fused_out }
            else { candle_core::bail!("Expected Fused output"); }
        } else {
            for block in &self.blocks {
                let res = block.forward(&x, temperature, None, None, false)?;
                if let BlockOutput::Standard(new_x) = res { x = new_x; }
                else { candle_core::bail!("Expected Standard output"); }
            }
            x
        };
