...
impl SequenceOp { pub fn new(max_len: usize, pad_val: i32) -> Self { Self { max_len, pad_val } } }

impl CustomOp for SequenceOp {
    fn name(&self) -> &str { "SequenceOp" }
    fn process(&self, inputs: &[Fv]) -> Result<Fv, String> {
        let seq = match &inputs[0] { Fv::IntList(l) => l, _ => return Err("Expected IntList".into()) };
