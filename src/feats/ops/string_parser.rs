...
impl super::CustomOp for StringParser {
    ...
    fn process(&self, inputs: &[&(dyn Any + Send + Sync)]) -> Result<Box<dyn Any + Send + Sync>, String> {
        let s = inputs[0].downcast_ref::<String>().map(|s| s.as_str()).unwrap_or("");
        let mut result: Vec<String> = s.split(&self.sep1).filter_map(|item| {
            let parts: Vec<&str> = item.split(&self.sep2).collect();
            if self.key_index < parts.len() { Some(parts[self.key_index].to_string()) } else { None }
        }).collect();
