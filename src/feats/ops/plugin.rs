...
type PluginFn = unsafe fn(&[&(dyn Any + Send + Sync)]) -> Result<Box<dyn Any + Send + Sync>, String>;

pub struct PluginOp { lib: Library, op_name: String }
...
impl CustomOp for PluginOp {
    ...
    fn process(&self, inputs: &[Fv]) -> Result<Fv, String> {
        ...
        unsafe {
            let func: libloading::Symbol<PluginFn> = self.lib.get(b"process_custom")
